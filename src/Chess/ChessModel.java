/******************************************************************************************
 * This class models the chess program, runs logic behind saving moves, undoing moves, and
 * moving for the AI
 *
...
public class ChessModel implements IChessModel {
    ...
    /*************************************************************************************
     * This constructor instantiates the board, pieces, game logic, as well as minor
     * counters and ArrayLists to create the logic behind the game of Chess
     *************************************************************************************/
    public ChessModel() {
        ...
        //For loop to set white pawns to save space
        for (int i = 0; i < 8; i++)
            board[6][i] = new Pawn(Player.WHITE);

        //Sets the rest of the white pieces
        ...
        //For loop to st black pawns to save space
        for (int i = 0; i < 8; i++) {
            board[1][i] = new Pawn(Player.BLACK);
        }

        //Sets the rest of the black pieces
    ...
    /**************************************************************************************
     * Returns whether the piece at location {@code [move.fromRow, move.fromColumn]} is
     * allowed to move to location {@code [move.fromRow, move.fromColumn]}.
     *
     ...
     * @throws IndexOutOfBoundsException if either {@code [move.fromRow, move.fromColumn]}
     * or {@code [move.toRow, move.toColumn]} don't represent valid locations on the board.
     *************************************************************************************/
    public boolean isValidMove(Move move) {
        if(move.fromRow < 0 || move.fromRow > 7 || move.toRow < 0
                || move.toRow > 7 || move.fromColumn < 0
    ...
    public void move(Move move) {
        ...
        if(move.fromRow < 0 || move.fromRow > 7 || move.toRow < 0 || move.toRow > 7
                || move.fromColumn < 0 || move.fromColumn > 7 || move.toColumn < 0
                || move.toColumn > 7)
            throw new IndexOutOfBoundsException();
        else{
                ...
                if ((board[row][col] != null
                    ...
                    for (int atrow = 0; atrow < 8; atrow++) {
                        for (int atcol = 0; atcol < 8; atcol++) {
                            ...
                            if(board[row][col] != null
                                    && board[row][col].isValidMove
                                    (new Move(row, col, atrow, atcol),board)){

                                //save and make the move
                                saveMove(row, col, atrow, atcol);
                                move(new Move(row, col, atrow, atcol));

                                // if the player isn't in check, undo move, return false
                                if (!inCheck(currentPlayer())){
                                    undoMove();
                                    return false;
                                }
    ...
    public void undoMove(){
        if (numMoves > 0) {
            ...
            else if (savedSpot.length() > 5 &&
                    savedSpot.charAt(savedSpot.length() - 1) == '@'){
    ...
    /**************************************************************************************
     * This method returns a boolean to represent whether you can use the castling method
     * in chess, after checking for empty pieces, first move booleans, as well as piece
     * type
        ...
        if (p == player2){
            ...
            if (!inCheck(p)) {
                ...
                if (board[7][4].type().equals("King") &&
                        board[7][4].player() == p && whiteKing) {

                    //Checks to see if left-Rook is in spot and is black
                    if (board[7][0].type().equals("Rook") &&
                            board[7][0].player() == p && whiteLeftRook) {
    ...
    /**************************************************************************************
     * This method performs the castling technique in chess if the respective canCastle
     * method returns true
     * @param p Representing the player that is attempting to castle
     *************************************************************************************/
        ...
        if (p == player2){
            ...
            if (!inCheck(p)) {
                ...
                if (board[0][7].type().equals("Rook") &&
                        board[0][7].player() == p && blackRightRook == true) {

                    //Checks location of king
                    if (board[0][4].type().equals("King") &&
                            board[0][4].player() == p && blackKing == true) {
            ...
            if (!inCheck(p)) {
                ...
                if (board[7][7].type().equals("Rook") &&
                        board[7][7].player() == p && whiteRightRook == true) {

                    //Checks location of king
                    if (board[7][4].type().equals("King") &&
                            board[7][4].player() == p && whiteKing == true) {
    ...
    public void castleRight(Player p){
        ...
        //Checks for player
        if (p == player2){
            //Double checks ability to castle given the player
            if (canCastleRight(p)){
    ...
    /***************************************************************************************
     * This method returns a boolean to represent whether you can use the En Passant method
     * in chess, after checking for certain pieces and certain moves done beforeso
     * @param p Representing the player that is attempting to use En Passant
     * @param move Representing the move being used to attempt En Passant
     * @return {@code true} For if you may use En Passant, but {@code false} if not able to
     *************************************************************************************/
    public boolean canEnPassant(Player p, Move move) {
        ...
        if(numMoves != 0) {
            ...
            if (Math.abs(pawnFromRow - pawnToRow) == 2) {
                ...
                if (board[pawnToRow][pawnToCol] != null &&
                        board[pawnToRow][pawnToCol].type().equals("Pawn")
                        && board[pawnToRow][pawnToCol].player() != p) {
                    if (move.fromRow == pawnToRow &&
                            move.toRow + direction == pawnToRow &&
                            move.toColumn == pawnToCol) {
    ...
    /**************************************************************************************
     * This method sets boolean values of false if the selected piece is moving for the
     * first time in the game. This method was created to help determine if you are
     * allowed to castle
     * @param m Representing the move about to be done
     *************************************************************************************/
    ...
    /**************************************************************************************
     * This method sets boolean values of true if the only if the undo move is moving one
     * of the respective pieces back to its original spot. This method was created to help
     * determine if you are allowed to castle.
     * @param m Representing the move about to be done
     *************************************************************************************/
    ...
    /**************************************************************************************
     * This method return the current player object
     * @return Return the current player, being white or black
     *************************************************************************************/
    ...
    /**************************************************************************************
     * This method returns the number of rows in our chess game
    ...
    /**************************************************************************************
     * This method return the number of columns in our chess game
    ...
    /**************************************************************************************
     * This method returns the IChessPiece located at the respective row and column,
     * if there is one
     * @return The IChessPiece at the row and column given
     * @param row Respective row of the piece
     * @param column Respective row of column
     *************************************************************************************/
    ...
    /**************************************************************************************
     * This method sets the next player after a turn
     *************************************************************************************/
    ...
    /**************************************************************************************
     * This method sets an IChessPiece of your choosing at the given row and column
     * @param row Respective row of the piece you are setting
     * @param column Respective column of the piece you are setting
     * @param piece The IChessPiece you would like to place at the given row and column
     *************************************************************************************/
    ...
    /*************************************************************************************
     *This method has the AI take moves for the black pieces. The order of logic
     * is listed below
     *
     * a. Check to see if you are in check.
     * 		i. If so, get out of check by moving the king, or placing a piece to
     * 		block the check
     *
     ...
     *		i. check to see if that piece is in danger of being removed, if so,
                    ...
                    for (int j = 0; j < 8; j++)
                        ...
                        for (int k = 0; k < 8; k++)
                            for (int l = 0; l < 8; l++) {
                                ...
                                //if the piece belongs to black, the move is valid
                                ...
                                if (board[i][j] != null &&
                                    ...
                                    //if black is still in check, undoes move and
                                    // does loop again
                                    if (inCheck(Player.BLACK)) {
                                        undoMove();
                                        goneYet = false;
                            ...
                            for (int l = 0; l < 8; l++) {
                                ...
                                if (board[i][j] != null && board[i][j].player() == Player.BLACK
                                    ...
                                    if (board[k][l] != null &&
                                            ...
                                            && !board[k][l].isValidMove
                                            (new Move(k, l, wKingX, wKingY), board)) {
                                        inDanger = true;
                                    }
                                    ...
                                    for (int m = 0; m < 8; m++) {
                                        for (int n = 0; n < 8; n++)
                                            if (board[m][n] != null && board[k][l] != null
                                                    ...
                                                    && board[m][n].isValidMove
                                                    (new Move(m, n, k, l), board)
                        ...
                        for (int j = 0; j < 8; j++)
                            ...
                            for (int k = 0; k < 8; k++)
                                for (int l = 0; l < 8; l++) {
                                    ...
                                    //checks to see if AI's piece can be targeted by black
                                    // and if AI has gone
                                    if (board[i][j] != null
                                            && board[i][j].player() == Player.WHITE
                                            && board[k][l] != null
                                            && board[k][l].player() == Player.BLACK
                                            && board[k][l].type().equals(types[type])
                                            && board[i][j].isValidMove
                                            (new Move(i, j, k, l), board)) {
                                        ...
                                        for (int typeTwo = 0; typeTwo < 5; typeTwo++) {
                                            ...
                                            for (int m = 0; m < 8; m++)
                                                for (int n = 0; n < 8; n++) {
                                                    ...
                                                    if (board[m][n] != null
                                                            && board[m][n].player() == Player.BLACK
                                                            && board[m][n].isValidMove
                                                            (new Move(m, n, i, j), board)
                                                            && board[m][n].type().equals
                                                            (types[4 - typeTwo]) && !goneYet) {
                                                        ...
                                                        for (int o = 0; o < 8; o++) {
                                                            for (int p = 0; p < 8; p++)
                                                                if (board[o][p] != null
                                                                        ...
                                                                        && board[o][p].player()
                                                                        == Player.WHITE
                                                                        && board[o][p].isValidMove
                                                                        (new Move(o, p, i, j), board)){
                                                                    inDanger = true;
                                                                }
                                                        }
                                                    ...
                                                    if (!goneYet) {
                                                        ...
                                                        //if the move is valid & our piece is of the right type
                                                                if (board[m][n] != null
                                                                        && board[m][n].player()
                                                                        == Player.BLACK
                                                                        && board[m][n].isValidMove
                                                                        (new Move(m, n, t, q), board)
                                                                        && board[m][n].type().equals
                                                                        (types[4 - typeTwo])
                                                                        && !goneYet) {
